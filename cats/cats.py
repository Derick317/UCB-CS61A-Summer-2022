...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def select(s):
        for word in split(remove_punctuation(s)):
...
def accuracy(typed, reference):
    ...
    correctness = [reference_words[i] == typed for i, typed in enumerate(typed_words) if i < len(reference_words)]
    return sum(correctness) / len(typed_words) * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if lowest_diff > limit:
        ...
        return word_list[diff.index(lowest_diff)]
    # END PROBLEM 5
...
def feline_fixes(typed, reference, limit):
    ...
    if len(reference) == 0:
        return len(typed)
    first_diff = (typed[0] != reference[0])
    return first_diff + feline_fixes(typed[1:], reference[1:], limit - first_diff)
    # END PROBLEM 6


def appear_once(typed, reference):
    """Does REFERENCE appear at least once in TYPED?"""
    if len(reference) == 0:
        return True
    if reference[0] not in typed:
        return False
    return appear_once(typed[typed.index(reference[0]) + 1:], reference[1:])
...
def hidden_kittens(typed, reference, limit):
    ...
    """
    # BEGIN PROBLEM 7
    if limit < 1 or len(reference) < 1:
        return 1
    if reference[0] not in typed:
        return limit + 1
    idx = typed.index(reference[0])
    # Now, that REFERENCE appears at least once in TYPED is equivalent to that REFERENCE[1:]
    # appears at least once in TYPED[idx + 1:]
    with_first = hidden_kittens(typed[idx + 1:], reference[1:], limit)
    if appear_once(typed[idx + 1:], reference):
        without_first = hidden_kittens(typed[idx + 1:], reference, limit - with_first)
    else:
        without_first = 0
    total = without_first + with_first
    return total if total else limit + 1
...
def final_diff(typed, reference, limit):
    ...
    return hidden_kittens(typed, reference, limit) + feline_fixes(typed, reference, limit)
...
def report_progress(typed, prompt, user_id, upload):
    ...
    for i, word in enumerate(typed):
        if word == prompt[i]:
            ...
            break
    upload({'id': user_id, 'progress': correct_num / len(prompt)})
    return correct_num / len(prompt)
    # END PROBLEM 8
...
def time_per_word(words, times_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    per_word = lambda l: [l[i + 1] - l[i] for i in range(len(l) - 1)]
    times = [per_word(l) for l in times_per_player]
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    result = [list() for _ in player_indices]
    for i in word_indices:
        fastest_player = 0
        for j in player_indices:
            if match["times"][j][i] < match["times"][fastest_player][i]:
                fastest_player = j
        result[fastest_player].append(match["words"][i])
    return result
    # END PROBLEM 10
