...
def merge(lst1, lst2):
    ...
    if lst2 == []:
        return lst1
    if lst1[-1] > lst2[-1]:
        return merge(lst1[:-1], lst2) + [lst1[-1]]
    else:
        return merge(lst1, lst2[:-1]) + [lst2[-1]]
...
class VendingMachine:
    ...
    'Here is your soda.'
    """
    def __init__(self, item: str, price) -> None:
        self._item = item
        self._price = price
        self._num = 0
        self._funds = 0
    ...
    def vend(self):
        ...
        if self._funds < self._price:
            return f"Please update your balance with ${self._price - self._funds} more funds."
        self._funds -= self._price
        self._num -= 1
        msg = f"Here is your {self._item}." if self._funds == 0 else f"Here is your {self._item} and ${self._funds} change."
        self._funds = 0
        return msg

    def add_funds(self, money):
        if self._num == 0:
            assert self._funds == 0
    ...
    def restock(self, num):
        self._num += num
        return f"Current {self._item} stock: {self._num}"
