def d_leaves_helper(t, v, d):
    for sub_tree in t.branches:
        d_leaves_helper(sub_tree, v, d + 1)
...
def has_path(t, target):
    ...
    if t.label != target[0] or not t.branches:
      return False
...
def deep_map(f, link):
    ...
    if link is Link.empty:
      return Link.empty
    current: Link = link
    result_first = deep_map(f, link.first) if isinstance(link.first, Link) else f(link.first)
    result_rest = deep_map(f, link.rest)
    return Link(result_first, result_rest)
