...
class Ant(Insect):
    ...
    def add_to(self, place):
        ...
        # BEGIN Problem 8b
            ...
            # If the ant originally occupying a place can contain the ant being added, then 
            # both ants occupy the place and original ant contains the ant being added.
            place.ant.store_ant(self)
        elif self.can_contain(place.ant):
            # If the ant being added can contain the ant originally in the space, then both 
            # ants occupy the place and the (container) ant being added contains the original ant.
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        dist = 0 # distance between plance and self.place
        while dist < self.lower_bound and not place.is_hive:
            place = place.entrance
            dist += 1
        while (self.upper_bound is None or dist <= self.upper_bound) and not place.is_hive:
            if place.bees:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew > 0:
            ...
            unluck_bee = random_bee(self.place.bees)
            unluck_bee.reduce_health(unluck_bee.health)
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        if self.slow_time > 0 and gamestate.time % 2:
            destination = None
        elif self.scare_time > 0:
            destination = None if self.place.entrance.is_hive else self.place.entrance
            self.scare_time -= 1
        else:
        ...
        if self.blocked() and self.slow_time == 0:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        if self.place.ant is not self:
            insects[self.place.ant] = 0
        for bee in self.place.bees:
                insects[bee] = 0
        place: Place = self.place.entrance
        while not place.is_hive:
            dist += 1
            for bee in place.bees:
                insects[bee] = dist
            if place.ant is not None:
                insects[place.ant] = dist
                if place.ant.is_container and place.ant.ant_contained is not None:
                    insects[place.ant.ant_contained] = dist
            place = place.entrance
        return insects
        # END Problem Optional 2
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        return max(2 - self.insects_shot / 16 - distance / 4, 0)
        # END Problem Optional 2
